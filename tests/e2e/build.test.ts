...
describe.skipIf(process.env.CI)("build command", () => {
    describe("less project", { sequential: true }, () => {
        ...
        it("should has scope", { timeout: 150_000 }, () => {
            const { all, failed } = execa({
                cwd: entry,
            })`${RUNNER} ${SCRIPT} build . --scope myscope`
            expect(failed, all).toBeFalsy()
            const pkgJson = readFileSync(
                toAbsolute("wasm-dist/package.json", toAbsolute(entry)),
                "utf8",
            )
        ...
        describe.skipIf(process.env.CI)("runtime", async () => {
            ...
            it(
                "index.js with async should be able to run in browser",
                { timeout: 20_000 },
                async () => {
            ...
            it(
                "index.js with sync should be able to run in browser",
                { timeout: 20_000 },
                async () => {
            ...
            it(
                "shims.js with async should be able to run in browser",
                { timeout: 20_000 },
                async () => {
            ...
            it(
                "shims.js with sync should be able to run in browser",
                { timeout: 20_000 },
                async () => {
    ...
    describe("more project", { sequential: true }, () => {
        it(
            "should generate shims with wrapper functions and type validations",
            { timeout: 150_000 },
            () => {
