...
describe.skipIf(process.env.CI)("build command", () => {
    it("should support entry as option", { timeout: 20_000 }, () => {
        const { failed } = execa`${RUNNER} ${SCRIPT} build fixture/less`
        expect(failed).toBeFalsy()
    })

    it("should support entry as command arg", { timeout: 20_000 }, () => {
        const { failed } = execa`${RUNNER} ${SCRIPT} build --entry fixture/less`
        expect(failed).toBeFalsy()
    })
